...
<head>
    ...
    <style>
        ...
        .status-em-aberto { background: #fff3cd; color: #856404; }
        .status-faturado { background: #d1edff; color: #0c5460; }
...
<body>
    ...
    // Resolve logo: prefer embedding as base64 for local files (avoids DOMPDF remote issues)
    $logoDataUri = null;
    if ($logo) {
        // If logo is absolute URL, keep as-is
        if (filter_var($logo, FILTER_VALIDATE_URL)) {
            $logoDataUri = $logo;
        } else {
            // Try to resolve an absolute filesystem path
            $possible = [];
            // If logo already contains IMG_PATH or similar, try relative to APPPATH or ROOTPATH
            ...
            // Last resort: use base_url to point to public path
            if (!$logoDataUri) {
                $logoDataUri = base_url($logo);
            }
        }
    }
    ...
    function formatStatus($status) {
        ...
        switch (strtolower($status)) {
            case 'em aberto': $class .= 'status-em-aberto'; break;
            case 'faturado': $class .= 'status-faturado'; break;
            case 'atrasado': $class .= 'status-atrasado'; break;
            case 'cancelado': $class .= 'status-cancelado'; break;
            default: $class .= 'status-em-aberto';
        }
    ...
    <?php if (!empty($vendas) && is_array($vendas)): ?>
        <table>
            <thead>
                <tr>
                    <th style="width: 12%;">Venda Nº</th>
                    <th style="width: 12%;">Data/Hora</th>
                    <th style="width: 10%;">Status</th>
                    <th style="width: 30%;">Cliente</th>
                    <th style="width: 20%;">Vendedor</th>
                    <th style="width: 16%; text-align: right;">Valor Total</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($vendas as $i => $v): ?>
                    <tr>
                        <td class="text-center"><?= esc($v['v1_numero_venda'] ?? ($v['id'] ?? '-')) ?></td>
                        <td class="text-center"><?= isset($v['data']) ? date('d/m/Y H:i', strtotime($v['data'])) : '-' ?></td>
                        <td class="text-center"><?= formatStatus($v['v1_status'] ?? '-') ?></td>
                        <td><?= esc($v['cliente_nome'] ?? '-') ?></td>
                        <td><?= esc($v['vendedor_nome'] ?? '-') ?></td>
                        <td class="text-right">R$ <?= number_format($v['valor_total'] ?? 0, 2, ',', '.') ?></td>
                    </tr>
                <?php endforeach; ?>
        ...
        <div class="summary">
            ...
            <div class="summary-item">
                ...
                <span class="summary-value">R$ <?= $totalVendas > 0 ? number_format($valorTotal / $totalVendas, 2, ',', '.') : '0,00' ?></span>
            </div>
        </div>
    ...
    <?php else: ?>
        <div class="no-data">
            ...
            Não foram encontradas vendas para o período e filtros selecionados.
        </div>
    <?php endif; ?>
